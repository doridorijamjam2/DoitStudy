...
<body>
	...
	if(!userID.equals(tip.getUserID())){
		...
		if (request.getParameter("tipTitle") == null || request.getParameter("tipContent") == null
				|| request.getParameter("tipTitle").equals("") || request.getParameter("tipContent").equals("")) {
			PrintWriter script = response.getWriter();
			script.println("<script>");
			...
			int result = tipDAO.update(tipID, request.getParameter("tipTitle"), request.getParameter("tipContent"));
			if (result == -1) {
		PrintWriter script = response.getWriter();
		script.println("<script>");
